...
#
# Writes: dist/Colin-McRae-Rally-2-0-Recomp-<VERSION>-<artifact-tag>.zip
set -euo pipefail
...
exec bash "${PACKAGER}" \
  ...
  --zip-prefix Colin-McRae-Rally-2-0-Recomp \
  --exe-name Colin_McRae_Rally_2_0_Recompiled \
  --display-name "Colin McRae Rally 2.0 Recompiled" \
  --recompiler-build "${RECOMPILER_BUILD}" \
  --version-env RELEASE_VERSION \
  --disc-hint "your legally owned Colin McRae Rally 2.0 disc" \
